...
$(document).ready(function(){
    ...
    grid.on('click', function(object) {
        if(object.rowKey != undefined){
        	   if(object.columnName == 'customerNum'){
...
function fn_detail(customerNum) {
    document.listForm.customerNum.value = customerNum;
    document.listForm.action = "<c:url value='/asset/customer/updateCustomerInfoView.do'/>";
    document.listForm.submit();
}

/* 글 등록 화면 function */
function fn_addCustomer() {
    document.listForm.action = "<c:url value='/asset/customer/addCustomerInfoView.do'/>";
    document.listForm.submit();
}
...
function fnSearch(){
    ...
    // search_select
    $.ajax({
        url : "<c:url value='/asset/customer/searchCustomerList.do'/>",
        type : "POST",
        dataType:'json',
    ...
    $.ajax({
         url : "<c:url value='/asset/customer/pageCustomerList.do'/>",
         type : "POST",
         dataType:'json',
         contentType:"application/json; charset=UTF-8",
         data : JSON.stringify(param),
...
function fn_select_page(page){
    ...
    $.ajax({
        url : "<c:url value='/asset/customer/CustomerListPage.do'/>",
        type : "POST",
        dataType:'json',
        contentType:"application/json; charset=UTF-8",
        data : JSON.stringify(param),
...
<form name="listForm" action ="<c:url value='/asset/customer/CustomerInfoList.do'/>" method="post">
   <div class="panel-body">
       <div class="search-area">
            <input type="hidden" name="customerNum"/>
   ...
   <div class="panel-footer justify-content-center">
       ...
       <div class="btn-area">
<!--            <button type="button" class="btn btn-primary btn-lg" onclick="fn_addCustomer(); return false;">등록</button> -->
       </div>
   </div>
</form>
