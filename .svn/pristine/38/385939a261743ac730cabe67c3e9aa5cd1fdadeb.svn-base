...
$(document).ready(function(){
    ...
    grid.on('click', function(object) {
        if(object.rowKey != undefined){
            fn_detail(grid.getValue(object.rowKey, "terminalManagenum"));
        }
    })
...
function fn_detail(terminalManagenum) {
    document.listForm.terminalManagenum.value = terminalManagenum;
    document.listForm.action = "<c:url value='/asset/terminal/updateTerminalInfoView.do'/>";
    document.listForm.submit();
}

/* 글 등록 화면 function */
function fn_addMeter() {
    document.listForm.action = "<c:url value='/asset/terminal/addTerminalInfoView.do'/>";
    document.listForm.submit();
}
...
function fnSearch(){
    ...
    // search_select
    $.ajax({
        url : "<c:url value='/asset/terminal/searchTerminalList.do'/>",
        type : "POST",
        dataType:'json',
    ...
    $.ajax({
         url : "<c:url value='/asset/terminal/pageTerminalList.do'/>",
         type : "POST",
         dataType:'json',
         contentType:"application/json; charset=UTF-8",
         data : JSON.stringify(param),
...
function fn_select_page(page){
    ...
    $.ajax({
        url : "<c:url value='/asset/terminal/TerminalListPage.do'/>",
        type : "POST",
        dataType:'json',
        contentType:"application/json; charset=UTF-8",
        data : JSON.stringify(param),
...
<form name="listForm" action ="<c:url value='/asset/terminal/TerminalInfoList.do'/>" method="post">
   <div class="panel-body">
       <div class="search-area">
            <input type="hidden" name="terminalManagenum"/>

            <label for="searchCondition" class="hidden">검색조건</label>
            <select id="searchCondition" name="searchCondition" title="검색조건 선택">
                <option value="">전체</option>
                <option value="1">단말기관리번호</option>
                <option value="2">고객번호</option>
                <option value="3">고객명</option>
                <option value="4">명칭</option>
            </select>
