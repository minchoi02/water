...
function fn_save_PopupManage(){
	...
	if(confirm("<spring:message code="common.save.msg" />")){
		varFrom.action =  "<c:url value='/sys/pwm/updtPopup.do' />";
...
<section class="panel-wrap">
    ...
    <form name="popupManageVO" method="post" >
	            ...
	            <table>
	                ...
	                <tbody>
	                    <tr>
	                        ...
	                        <td>
	                            <div>
	                                <input name="popupTitleNm" type="text" maxlength="255" value="${popupManageVO.popupTitleNm}" />
	                            </div>
	                        </td>
	                    </tr>
	                    <tr>
                                                ...
                                                var onCloseMonitorWindowCu = function () {
                                                    ...
                                                    if (uploader.getUploadStatus() == 'COMPLETION') {
									                    ...
									                    */
									                    ...
									                    document.popupManageVO.action = "<c:url value='/sys/pwm/updtPopup.do'/>";
									                    document.popupManageVO.submit();
	                    ...
	                    <tr>
	                        ...
	                        <td>
	                            <div>
	                                <input name="popupDc"  type="text" placeholder="입력하세요." value="${popupManageVO.popupDc}" />
	                            </div>
	                        </td>
	                    </tr>
	                    <tr>
	                        ...
	                        <td>
	                            <div>
	                                <input name="fileUrl" type="text" maxlength="255" size="35"  value="${popupManageVO.fileUrl}"/>
	                            </div>
	                        </td>
	                    </tr>
	                    <tr>
	                        ...
	                        <td>
	                            <div class="label-input">
	                              	<label><input id="ntceAt" type="radio" name="ntceAt" value="Y" <c:if test="${popupManageVO.ntceAt eq 'Y'}">checked</c:if>> 사용</label>
	                                <label><input type="radio" name="ntceAt" value="N" <c:if test="${popupManageVO.ntceAt eq 'N'}">checked</c:if>>미사용</label>
	                            </div>
	                        </td>
	                    </tr>
	                    <tr>
	                            ...
	                            <div class="col-6 calendar-day mt15">
									...
									<select  id="ntceBgndeHH"  name="ntceBgndeHH">
										<c:forEach var="obj" items="${ntceBgndeHH}">
									  		<option <c:if test="${popupManageVO.ntceBgndeHH eq obj.code}">selected="selected"</c:if> value="${obj.code}">${obj.code}</option>
									  	</c:forEach>
									</select>
									</div>
									 :
									...
									<select id="ntceBgndeMM" name="ntceBgndeMM">
										<c:forEach var="obj" items="${ntceBgndeMM}">
									  		<option <c:if test="${popupManageVO.ntceBgndeMM eq obj.code}">selected="selected"</c:if> value="${obj.code}">${obj.code}</option>
									  	</c:forEach>
									</select>
									</div>
									&nbsp;~&nbsp;
									...
									<select id="ntceEnddeHH" name="ntceEnddeHH" >
										<c:forEach var="obj" items="${ntceEnddeHH}">
									  		<option <c:if test="${popupManageVO.ntceEnddeHH eq obj.code}">selected="selected"</c:if> value="${obj.code}">${obj.code}</option>
									  	</c:forEach>
									</select>
									</div>
									:
									...
									<select id="ntceEnddeMM" name="ntceEnddeMM">
										<c:forEach var="obj" items="${ntceEnddeMM}">
									  		<option <c:if test="${popupManageVO.ntceEnddeMM eq obj.code}">selected="selected"</c:if> value="${obj.code}">${obj.code}</option>
									  	</c:forEach>
									</select>
									</div>
									</div>
	    ...
	    <input type="hidden" id="atchFileId" name="atchFileId" value="${popupManageVO.atchFileId}"  />
    ...
    <div class="panel-footer">
        <div class="btn-area">
            <button type="button" class="btn btn-primary btn-lg" onclick="javascript:fn_save_PopupManage(); return false;">수정</button>
            <button type="button" class="btn btn-gray btn-lg" onclick="javascript:fn_delete_PopupManage(); return false;">삭제</button>
