...
function fn_egov_ZipSearch(frm, sZip, vZip, sAddr) {
	...
	// FIREFOX
	var openParam = "dialogWidth:590px;dialogHeight:550px;scroll:no;status:no;center:yes;resizable:yes;";
...
function zipCallback(retVal) {
	if (retVal) {
		otherParameters[0].value  = retVal.zipSn;
		otherParameters[1].value  = retVal.zipCd;
		otherParameters[2].value = retVal.addr;
	}
}
