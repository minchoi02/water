...
<!--
    ...
    function goMenuPage(baseMenuNo){
        ...
        var link = getLastLink(baseMenuNo);

        if(/^http/.test(link)){
            document.menuListForm.target = "_blank"
    	    document.menuListForm.action = link;
        }else{
    	    document.getElementById("link").value="forward:"+link;
...
<%--         <li><a href="#LINK" onclick="javascript:goMenuPage('<c:out value="${result.menuNo}"/>')"><c:out value="${result.menuNo}"/></a></li> --%>
<%--         </c:if> --%>
...
<!-- <a href="#LINK" class="top_issue">지정서·등록증 발급</a> -->
