...
$(document).ready(function(){
    ...
    function fn_regist_notice() {
        ...
        if (confirm('<spring:message code="common.regist.msg" />')) {
            ...
            if(uploader.fileInfoMap.size!=1){
                alert('대표사진을 등록해주세요');
                return false;
            }else{
    ...
    function fn_select_noticeList() {
		//document.board.action = "<c:url value='/cop/bbs/selectBoardList.do'/>"+ "?bbsId=" +"<c:out value='${boardVO.bbsId}'/>" + "&bbsNm=" + "<c:out value='${boardVO.bbsNm}'/>";
