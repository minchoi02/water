...
public class ProgrmManageServiceImpl extends EgovAbstractServiceImpl implements ProgrmManageService {
	...
	public ProgrmManageVO selectProgrm(String progrmNm) throws Exception{
         	return progrmManageDAO.selectProgrm(progrmNm);
	}
	/**
	 * 프로그램 목록을 조회
	 * @param vo ComDefaultVO
	...
	public List<?> selectTreeCodeList(AuthorVO vo) throws Exception {
   		return progrmManageDAO.selectTreeCodeList(vo);
	}
	@Override
	public List<?> selectAuthorTreeList(AuthorVO vo) throws Exception {
		return progrmManageDAO.selectAuthorTreeList(vo);
	}
	@Override
	public List<?> selectAuthorList() throws Exception {
		return progrmManageDAO.selectAuthorList();
	}
	@Override
	public void updateTreeCode(List<AuthorVO> voList) throws Exception {


		progrmManageDAO.deleteTreeCode(voList.get(0));

		for(int i=0;i<voList.size();i++) {
			AuthorVO vo = voList.get(i);
			if(!vo.getParent().equals("#")) {
				progrmManageDAO.insertTreeCode(vo);
			}
		}
	}
}