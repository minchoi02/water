...
<%
	...
	{	
			...
			{
				session.invalidate(); 
				result_sc = "<script>alert(NamoSELang.pe_zs);window.document.location.href='../index.html';</script>";
			}
			else
			{
				result_sc = "<script>alert(NamoSELang.pe_vx+'" + fileName + "');history.back();</script> ";
			}
	}
	else{
		result_sc = "<script>alert(NamoSELang.pe_Bz);location.href='account_setting.jsp';</script>";
	}
