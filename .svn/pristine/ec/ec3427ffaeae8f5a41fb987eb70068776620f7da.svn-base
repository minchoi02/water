...
function fnFind() {
    ...
    $.ajax({
        url:'<c:url value='/cmm/login/checkNmPhone.do'/>',
        type:'POST',
        dataType:'json',
        ...
        success:function(data){
            if(data.userId ==null){
                $("#message").html('입력하신 이름 및 전화번호에 해당하는 아이디는 없습니다');
            }else{
                $("#message").html('아이디는 '+data.userId+' 입니다');
            }
        }
    });
}
...
<div class="login-area">
	...
	<div class="login-box">
		...
		<form name="checkForm" action ="<c:url value='/sys/user/urm/IdFindAction.do'/>">
				<fieldset>
					<legend id="login" class="hidden">아이디 찾기</legend>
					<ul class="row">
						<li class="col-12">
						...
						<li class="col-12">
							<button type="button" class="btn btn-secondary" onclick="javascript:fnFind();">찾기</button>
						</li>
					</ul>
				</fieldset>
			</form>
