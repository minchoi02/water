...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/sys/bwm/listBanner.do'/>";
   	document.listForm.submit();
}
/* ********************************************************
 * 등록 처리 함수
 ******************************************************** */
function fn_regist_BannerManage(){
	var vFrom = document.listForm;
	vFrom.action = "<c:url value='/sys/bwm/registBanner.do' />";
	vFrom.submit();
}
/* ********************************************************
 * 상세화면 처리 함수
 ******************************************************** */
function fn_detail_BannerManage(bannerId){
	var vFrom = document.listForm;
	vFrom.bannerId.value = bannerId;
	vFrom.action = "<c:url value='/sys/bwm/updtBanner.do' />";
	vFrom.submit();
}
 ...
 * 검색 함수
 ******************************************************** */
function fn_search_BannerManage(){
	var vFrom = document.listForm;

	vFrom.action = "<c:url value='/sys/bwm/listBanner.do' />";
	vFrom.submit();
...
* 체크 박스 선택 함수
******************************************************** */
function fn_checkAll_BannerManage(){
...
function fn_view_BannerManage(){
	...
	if( FLength == 1){
		if(document.getElementById("checkList").checked == true){
			fn_ajaxBannerInfo_BannerManage( document.getElementById("checkList").value );
			check ++;
		}
	}else{
		for(var i=0; i < FLength; i++)
		{
			if(document.getElementsByName("checkList")[i].checked == true){
				check ++;
				fn_ajaxBannerInfo_BannerManage( document.getElementsByName("checkList")[i].value );
				}
		}
		}
...
/* ********************************************************
* 배너창 정보 Ajax통신으로 정보 획득
******************************************************** */
function fn_ajaxBannerInfo_BannerManage(bannerIds){
	var url = "<c:url value='/sys/bwm/ajaxBannerManageInfo.do' />";
	...
	new Ajax.Request(url,
   {
    ...
    onSuccess  : function(returnValue)
    {
    	...
    	//if(fnGetCookie(bannerIds) == null ){
    	   	fn_bannerOpen_BannerManage(bannerIds,
        	    	returnValueArr[0],
        	    	returnValueArr[1],
        	    	returnValueArr[2],
        	    	returnValueArr[3],
...
* 배너창  오픈
******************************************************** */
function fn_bannerOpen_BannerManage(bannerId,fileUrl,width,height,top,left,stopVewAt){

	var url = "<c:url value='/sys/bwm/openBannerManage.do' />?";
...
<section class="panel-wrap">
    ...
    <form name="listForm" action="<c:url value='/sys/bwm/listBanner.do'/>" method="post">
	    ...
	    <div class="panel-body">
	        <div class="search-area">
	                ...
	                <button type="button" onclick="javascript:fn_search_BannerManage()" class="btn btn-primary">검색</button>
	        </div>
	            ...
	            <table>
	                ...
	                <tbody>
	                   <c:forEach var="resultInfo" items="${resultList}" varStatus="status">
	                    <tr>
	                        <td><input type="checkbox" name="checkList" id="checkList" value="${resultInfo.bannerId}"</td>
	                        <td>
	                            <dl class="list-area-dl">
	                                <dt class="badge bg-lightgray">배너명</dt>
	                                <dd><a href="#LINK" onclick="fn_detail_BannerManage('${resultInfo.bannerId}'); return false;"><c:out value="${resultInfo.bannerTitleNm}"/></a></dd>
	                                <dt class="badge bg-lightgray">URL</dt>
	                                <dd><c:out value="${resultInfo.fileUrl}"/></dd>
	                                <dt class="badge bg-lightgray">연결</dt>
	                                <dd>새창</dd>
	                                <dt class="badge bg-lightgray">클릭</dt>
	                                <dd>3회</dd>
	                            </dl>
	                        </td>
    ...
    <div class="panel-footer">
        ...
        <div class="btn-area">
            <button type="button" class="btn btn-primary btn-lg" onclick="javascript:fn_regist_BannerManage(); return false;">등록</button>
        </div>
    </div>
</section>
<!-- panel -->