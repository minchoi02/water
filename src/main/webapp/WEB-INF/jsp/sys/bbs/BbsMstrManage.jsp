...
$(document).ready(function(){
    ...
    class TextCustomRenderer {
        constructor(props){
            var el = document.createElement('div');
            var text = document.createTextNode('글보기');
            el.appendChild(text);
    ...
    grid.on('click', function(object) {
        if(object.rowKey!=undefined){
        	if(object.columnName != 'goBbs'){
        		fnDetail(grid.getValue(object.rowKey,"bbsId"));
        	} else {
        		fn_goBbs(grid.getValue(object.rowKey, "bbsId"));
        	}
...
function linkPage(pageNo){
    ...
    document.listForm.action = "<c:url value='/sys/bbs/BbsMstrManage.do'/>";
    document.listForm.submit();
}
function fnExcel(){
	document.listForm.action = "<c:url value='/sys/bbs/BbsMstrManageExcel.do'/>";
	document.listForm.submit();
}
/* ********************************************************
 * 등록 처리 함수
...
function fnRegist(){
    ...
    document.listForm.action           = "<c:url value='/sys/bbs/BbsMstrInsertView.do'/>";
    document.listForm.submit();
}
/* ********************************************************
 * 상세화면 처리 함수
 ******************************************************** */
function fnDetail(bbsId){
    document.listForm.action           = "<c:url value='/sys/bbs/BbsMstrDetailSelect.do'/>"+"?bbsId=" + bbsId;
    document.listForm.submit();
}
function fn_goBbs(bbsId) {


	document.listForm.baseMenuNo.value="6000000";
	document.listForm.link.value="forward:/cop/bbs/selectBoardList.do?bbsId=" + bbsId;
	document.listForm.action = "<c:url value='/PageLink.do'/>";
	document.listForm.submit();
}
...
function fnSearch(){
    ...
    // search_select
    $.ajax({
        url : "<c:url value='/sys/bbs/searchBbsMstrManage.do'/>",
        type : "POST",
        dataType:'json',
    ...
    $.ajax({
         url : "<c:url value='/sys/bbs/pageBbsMstrManage.do'/>",
         type : "POST",
         dataType:'json',
         contentType:"application/json; charset=UTF-8",
         data : JSON.stringify(param),
...
function fn_select_page(page){
    ...
    $.ajax({
        url : "<c:url value='/sys/bbs/BbsMstrManagePage.do'/>",
        type : "POST",
        dataType:'json',
        contentType:"application/json; charset=UTF-8",
        data : JSON.stringify(param),
...
<section class="panel-wrap">
    ...
    <form name="listForm" action ="<c:url value='/sys/bbs/BbsMstrManage.do'/>" method="post">
	    <div class="panel-body">
	        <div class="search-area">
                <input name="searchCondition" id="searchCondition" type="hidden" value="1" />
                <input name="baseMenuNo" id="baseMenuNo" type="hidden" />
...
<!-- 	            <div> -->
<!-- 	                <select> -->
<!-- 	                    <option value="1">데이터 사용</option> -->
<!-- 	                    <option value="2">데이터 미사용</option> -->
<!-- 	                    <option value="3">데이터 삭제</option> -->
<!-- 	                </select> -->
...
<!-- 	            </div> -->
	        ...
	        <div class="panel-footer justify-content-center ml0 mr0">
	            ...
	            <div class="btn-area">
	                <button type="button" class="btn btn-primary btn-lg" onclick="javascript:fnRegist(); return false;">등록</button>
	            </div>
	        </div>
	    </div>
    </form>
