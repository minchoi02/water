...
<script type="text/javascript">
	...
	function actionLogin() {
		if (document.loginForm.admin_id.value =="") {
	        ...
	        $("#admin_id").focus();
	        return false;
	    } else if (document.loginForm.admin_pw.value =="") {
	        alert("패스워드를 입력하세요");
	        $("#admin_pw").focus();
	        return false;
	    } else {
	        $("#loginForm").attr('action', '/admin/login/loginproc').submit();
	    }
	}
        ...
        <div class="login-body">
            <form id="loginForm" name="loginForm"  method="POST">
                <fieldset>
                    ...
                    <input type="text" id="admin_id" name="admin_id" class="mb-gutter" placeholder="이메일 아이디" />
                    <label for="pw1" class="hidden">패스워드</label>
                    <input type="password" id="admin_pw" name="admin_pw" placeholder="패스워드" onkeydown="javascript:if (event.keyCode == 13) { actionLogin(); }"/>
...
<!--                 <li><a href="#LINK" onclick="fnFindIdPw();">패스워드찾기</a></li> -->
<!--             </ul> -->
        </div>
    </div>
</div>
