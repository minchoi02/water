...
function fnMyPage(uniqId) {
    ...
    <div class="header-logo">
        <c:if test="${LoginVO.authorCode == 'ROLE_ADMIN' }">
            <h1><a href="<c:url value='/admin/main/mainPage.do'/>" title = "상수도 원격검침 시스템(관리자)"><em class="hidden">상수도 원격검침 시스템(관리자)</em></a></h1>
        </c:if>
        <c:if test="${LoginVO.authorCode != 'ROLE_ADMIN' }">
            <h1><a href="<c:url value='/cmm/main/mainPage.do'/>" title = "상수도 원격검침 시스템(관리자)"><em class="hidden">상수도 원격검침 시스템(관리자)</em></a></h1>
        </c:if>
    </div>
...
<!--           <li>등록된 메뉴가 없습니다.</li> -->
    ...
    <ul class="user-wrap">
        ...
        <li><a href="#" onclick="document.getElementById('logout-form').submit();"><i class="fas fa-sign-out-alt"></i>LOGOUT</a>
	        <form id="logout-form" action='<c:url value='/admin/login/logout.do'/>' method="POST">
                        ...
                        <table>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>댓/답글</td>
                                    <td>작성일시</td>
                                    <td><a href="/cop/bbs/selectBoardList.do?bbsId=BBS01" class="btn btn-default btn-auto" title="새창으로 열립니다."><i class="fas fa-external-link-alt"></i></a></td>
                                </tr>
                                <tr>
                                    <td>4</td>
                                    <td>캘러리</td>
