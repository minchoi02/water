...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			vHtmlCodeBg      ="<li class=\"depth-none\" id='" + i + "'>";
			vHtmlCodeBgList  ="<li id='" + i + "'>";
		...
		/*	if (hasChildNode) {
			...
			if (hasChildNode) {
			        vHtmlCode +=vHtmlCodeBgList+"<a href=#div" + nodeValues[0]+" data-toggle=\"collapse\">"+nodeValues[2]+"</a>";
				vHtmlCode +="<ul class='collapse' id='div" + nodeValues[0] + "'";
