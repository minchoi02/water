...
function createTree(arrName ) {
   var startNode, openNode;
	...
	if (treeNodes.length > 0) {
		...
		} else document.write("<input type='checkbox' name='checkAll' class='check2' onclick='javascript:fCheckAll();'><span style='font-weight:bold;'>&nbsp;메뉴목록</span><br>");
...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			document.write("<a href=javascript:parent.temp_aa('" + treeNodes[i] + "'); style='color:black;'>");
